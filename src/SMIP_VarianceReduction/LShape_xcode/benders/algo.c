...
int algo (oneProblem *orig, timeType *tim, stocType *stoc, string probName) {
    ...
    double   compromise_time=0, lb_mean=0, lb_std=0;
    double ub_mean=0, ub_std=0;
    batchSummary *batch = NULL;
	int 	 rep, m, n, out_idx=0;
	FILE 	*sFile, *iFile = NULL, *fFile=NULL;
...
//    if ( config.MASTER_TYPE == PROB_QP )
	...
	for ( rep = 0; rep < config.NUM_REPS; rep++ ) {
		...
		if (config.EVAL_FLAG == 1) {
			if ( config.MASTER_TYPE == PROB_QP )
				evaluate(sFile, stoc, prob, cell, cell->incumbX, &ub_mean, &ub_std);
			else
				evaluate(sFile, stoc, prob, cell, cell->candidX, &ub_mean, &ub_std);
		}
        ...
        if (!equalVector(meanSol, cell->candidX, prob[0]->num->cols, config.TOLERANCE)){
            fprintf(sFile, "\n-------------------------mean solution-------------------------\n");
              ...
              evaluate(sFile, stoc, prob, cell, meanSol, &ub_mean, &ub_std);
        }
#endif
    ...
    if ( config.MULTIPLE_REP ) {
        ...
        fprintf(fFile, "Incumbent solution, non-zero position: ");
        printVectorInSparse(batch->compromiseX, prob[0]->num->cols, fFile);
        evaluate(fFile, stoc, prob, cell, batch->compromiseX, &ub_mean, &ub_std);
        
        
        fprintf(fFile, "Time to solve compromise problem   : %f\n", compromise_time);
        ...
        fprintf(fFile, "New Lower bound estimate               : %f\n", batch->Est);
        LowerBoundVariance(batch, &lb_mean, &lb_std);
        fprintf(fFile, "Old Lower bound estimation             : %f(+-%f)\n", lb_mean, lb_std);
        
        fprintf(fFile, "\n\n Opt. Time, Cpm. Time, Cpm. LB, UB, Reduce Variance");
        fprintf(fFile, "\n %.3f     %.3f  %.3f  [%.3f, %.3f]     %.2f%%\n",batch->time->repTime, compromise_time, batch->Est, ub_mean-1.96*ub_std, ub_mean+1.96*ub_std, 100.0 - 100.0/sqrt(batch->max_repeat));
        
        fprintf(fFile, "\n------------------------------------------- Average solution ---------------------------------------\n\n");
        fprintf(stdout, "\n------------------------------------------- Average solution ---------------------------------------\n\n");
        /* Evaluate the average solution */
        printVectorInSparse(batch->avgX, prob[0]->num->cols, fFile);
        evaluate(fFile, stoc, prob, cell, batch->avgX, &ub_mean, &ub_std);
        ...
        fprintf(fFile, "\n---------------------------bagging output-------------------------- \n");
        fprintf(fFile, "bagging solution: \n");
        ...
        if (equalVector(batch->compromiseX, batch->distinctX[batch->bagging_idx], prob[0]->num->cols, config.TOLERANCE)){
            ...
            evaluate(sFile, stoc, prob, cell, batch->distinctX[batch->bagging_idx], &ub_mean, &ub_std);
